...
from scripts.models.nn_auv import AUVPROXYDeltaV, AUVTraj
...
def integrate(data_dir, tau=50, visu=True):
    ...
    dv_pred_proxy = AUVPROXYDeltaV(Bdvs[:, 1:]).to(device)
    pred.auv_step.dv_pred = dv_pred_proxy

    pose = trajs[:, :1].to(device)
    vel = Bvels[:, :1].to(device)
    
    s = time.time()
    # predict up to the last state as we don't know tau+1
    pred_trajs, pred_vs, pred_dvs = pred(pose, vel, acts[:, :tau])
    e = time.time()
    print(f"Prediciton time: {e-s}")

    loss_fc = TrajLoss().to(device)
    l = loss_fc(trajs[:, 1:tau+1], pred_trajs,
                Bvels[:, 1:tau+1], pred_vs,
                Bdvs[:, 1:tau+1], pred_dvs)
    ...
    if visu:
        ...
        plot_traj({"pred": pred_traj_euler, "gt": traj_euler[1:tau+1]}, s_col, tau, True, title="State")
        v_col = {"u": 0, "v": 1, "w": 2, "p": 3, "q": 4, "r": 5}
        plot_traj({"pred": pred_vs[0], "gt": Bvels[0, 1:tau+1]}, v_col, tau, True, title="Velocities")
        dv_col = {"Bdu": 0, "Bdv": 1, "Bdw": 2, "Bdp": 3, "Bdq": 4, "Bdr": 5}
        plot_traj({"pred": pred_dvs[0], "gt": Bdvs[0, 1:tau+1]}, dv_col, tau, True, title="Velocities Deltas")
        plt.show()