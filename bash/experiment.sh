...
cd ../scripts
for lam in 0.05 0.07 0.1 0.15 0.2
do
    for upsi in 1 1.4 1.8 2.0
    do
        for gam in 0.05 0.07 0.1 0.15 0.2
        do
            for noise in 0.10 0.15 0.20
            do
                python gen_config.py ../config/envs/point_mass2d.yaml elipse -l $lam -g $gam -u $upsi -n $noise
                python main.py --new --config /tmp/config.yaml --task /tmp/task.yaml -s 300 -l ## -g ../gifs/exp_l$lam_g$gam_u$upsi_n$noise.gif
            done
        done
    done
done
