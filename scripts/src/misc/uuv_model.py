...
class Vehicle(object):
    ...
    def __init__(self, inertial_frame_id='world', param_dict=None, dt=0.1):
        """Class constructor."""
        ...
        if "mass" in param_dict:
            self._mass = param_dict["mass"]

        self._volume = 0.0
        if "volume" in param_dict:
            self._volume = param_dict["volume"]

        self._density = 1028.0
        if "denisty" in param_dict:
            self._density = param_dict["density"]

        self._cog = [0., 0., 0.]
        if "cog" in param_dict:
            self._cog = param_dict["cog"]

        self._cob = [0., 0., 0.]
        if "cob" in param_dict:
            self._cob = param_dict["cob"]

        self._height = 0.
        if "height" in param_dict:
            self._height = param_dict["height"]

        self._length = 0.
        if "length" in param_dict:
            self._length = param_dict["length"]

        self._width = 0.
        if "length" in param_dict:
            self._width = param_dict["width"]

        self._inertial = dict(ixx=0, iyy=0, izz=0, ixy=0, ixz=0, iyz=0)
        if "inertial" in param_dict:
            inertial_arg = param_dict["inertial"]
            for key in self._inertial:
                if key not in inertial_arg:
                    raise AssertionError("Invalid moment of inertia format")
            self._inertial = inertial_arg

        # Loading the linear damping coefficients
        self._linear_damping = np.zeros(shape=(6, 6))
        if "linear_damping" in param_dict:
            self._linear_damping = np.array(param_dict["linear_damping"])
            if self._linear_damping.shape == (6,):
                self._linear_damping = np.diag(self._linear_damping)
            assert (self._linear_damping.shape == (6, 6)), "Linear damping must be\
        ...
        if "quad_damping" in param_dict:
            self._quad_damping = np.array(param_dict["quad_damping"])
            assert (self._quad_damping.shape == (6,)), "Quadratic damping must\
                                                        be defined with 6\
                                                        coefficients"

        self._linear_damping_forward_speed = np.zeros(shape=(6, 6))
        if "linear_damping_forward_speed" in param_dict:
            self._linear_damping_forward_speed = np.array(
                param_dict["linear_damping_forward_speed"])
            ...
            assert (self._linear_damping_forward_speed.shape == (6, 6)), "Linear damping\
                                                                          proportional to the\
                                                                          forward speed must\
                                                                          be given as a 6x6\
        ...
        if "Ma" in param_dict:
            self._Ma = np.array(param_dict["Ma"])
    ...
    def _update_restoring(self, q=None, use_sname=False):
        ...
        Fbb = np.dot(rotItoB, Fb)
        Fbg = np.dot(rotItoB, Fg)
        Mbb = np.cross(self._cob, Fbb)
        Mbg = np.cross(self._cog, Fbg)
    ...
    def compute_force(self, acc=None, vel=None, with_restoring=True, use_sname=True):
        ...
        self._update_restoring(use_sname=False)
    ...
    def compute_acc(self, gen_forces=None, use_sname=True):
        ...
        if self._Mtotal.sum() == 0:
            ...
            # print("C:  ", self._C.shape)
            # print("D:  ", self._D.shape)
            # print("g:  ", self._g.shape)
            # print("nu: ", nu.shape)
            # print("gen_forces: ", self._gen_forces.shape)

            # print("lhs: ", self._Mtotal.shape)
            # print("rhs: ", (self._gen_forces - np.dot(self._C, nu) - np.dot(self._D, nu) - self._g).shape)
    ...
    def step(self, state, action, dev=False):
        pos = state[0:3]
        quat = state[3:7]
        vel = state[7:]

        gen_forces = action
        p_next, q_next, v_next = self.internal_step(pos, quat, vel, gen_forces)
        next_state = np.concatenate([p_next, q_next, v_next], axis=0)
    ...
    def internal_step(self, pos, quat, vel, gen_forces):
        # assume quaternion respresentation of the angles
        self.pos = pos # world frame
        self.quat = quat # world frame
        self.vel = vel # body frame
        v_dot = self.compute_acc(gen_forces, False) #body frame

        p_dot = np.dot(self.rotBtoI, vel[0:3])
        q_dot = np.dot(self.TBtoIquat, vel[3:])

        p_next = self.pos + self._dt*p_dot
        q_next_unnormed = self.quat + self._dt*q_dot
        q_next = q_next_unnormed / np.linalg.norm(q_next_unnormed)
        
        v_next = self.vel + self._dt*v_dot

        return p_next, q_next, v_next