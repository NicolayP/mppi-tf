...
class ModelBase(object):
    def __init__(self, mass=1, dt=0.1, state_dim=2,
                 act_dim=1, name="point_mass"):
    ...
    def train_step(self, gt, x, a, step, writer=None, log=False):
        ...
        if log:
            with writer.as_default():
                tf.summary.scalar("training/mass",
                                  self.mass.numpy()[0, 0],
                                  step=step)
...
def main():
    ...
    _ = model.buildStepGraph("model", state, action)
    ...
    plot(gt_plot[:, -1], x_plot[:, -1], a_plot,
         0.25, model.mass.numpy()[0], 0.1)
