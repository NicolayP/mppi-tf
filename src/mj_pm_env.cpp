...
PointMassEnv::PointMassEnv(const char* envFile, const char* mjkey, bool view) {
  std::cout << glfwGetVersionString() << std::endl;
  info = string("Point Mass Environment in mujoco");
  ...
  m = mj_loadXML(envFile, NULL, NULL, 0);
...
bool PointMassEnv::simulate(vector<float> u) {
...
void PointMassEnv::step(vector<float>& x, const vector<float>& u) {
    for (int i=0; i < m->nu; i++) {
        d->ctrl[i] = u[i];
    }
    mj_step(m, d);
...
void PointMassEnv::get_x(vector<float> &x) {
    for (int i=0; i < m->nq; i++) {
        x[i] = d->qpos[i];
    }
